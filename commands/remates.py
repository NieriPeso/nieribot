    ...
    try:
        ...
        if not datos[1].startswith('nombre ') and not datos[2].startswith('descripcion ') and not datos[3].startswith('base ') and not datos[4].startswith('final '):
            ...
            if len(datos[4]) > 6 and datos[4][8] == '/' and datos[4][11] == '/' and datos[4][14] == ' ' and datos[4][17] == ':':
                ahora = datetime.now(tz=tz).strftime('%d/%m/%y %H:%M').split(' ')
                final = str(datos[4][6:])
                if int(ahora[0].split('/')[0]) > int(final.split(' ')[0].split('/')[0]) or int(ahora[0].split('/')[1]) > int(final.split(' ')[0].split('/')[1]) or int(ahora[0].split('/')[2]) > int(final.split(' ')[0].split('/')[2]) or int(ahora[0].split('/')[0]) >= int(final.split(' ')[0].split('/')[0]) and int(ahora[0].split('/')[1]) >= int(final.split(' ')[0].split('/')[1]) and int(ahora[0].split('/')[2]) >= int(final.split(' ')[0].split('/')[2]) and int(ahora[1].split(':')[0]) >= int(final.split(' ')[1].split(':')[0]) and int(ahora[1].split(':')[1]) + 10 > int(final.split(' ')[1].split(':')[1]):
                    embed = discord.Embed(
